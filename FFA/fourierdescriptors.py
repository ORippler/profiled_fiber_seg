...
if __name__ == "__main__":
    ...
    arrays = []
    bpath = "/work/local/rippel/ba_schnabel/Code/Output/automated_segmentation/normal_pinhole/"
    for array in os.listdir(bpath):
        if ".npy" in array:
            arrays.append(os.path.join(bpath, array))
    ...
    for array in arrays:
        ...
        name = os.path.split(array)[-1].strip(".npy") + ("_")
        coeffs, cnts, rots = extract_cnts_coeffs_from_mask(
            img, size_invariant=True
        )
        reduced_cnts = reduce_cnts_based_on_coeffs(coeffs, cnts, percentile=50)
        ...
        plt.gcf().savefig(name + "original_contours.png")

        fig, ax = plt.subplots()
        for n, contour in enumerate(reduced_cnts):
            ax.plot(contour[:, 0], contour[:, 1], linewidth=1)
        plt.gca().invert_yaxis()
        plt.gca().set_xlim(0, shape[1])
        plt.gca().set_ylim(0, shape[0])
        plt.gcf().savefig(name + "reduced_cnts.png")
        plt.close("all")
